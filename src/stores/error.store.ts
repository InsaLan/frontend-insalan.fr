...
export const useErrorStore = defineStore('error', () => {
	const errors: Array<Array<string>> = reactive([]);
	 ...
	 * @param error: string | string [] | ErrorMessage | ErrorMessage[] | {data: ErrorMessage}
	...
	let has_errors = computed(() => {return errors.length > 0 })
	return { errors,
		 add_error,
		 flush_errors,
		 has_errors
